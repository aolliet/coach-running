    ...
    <Layout>
          ...
          <div className="h-full overflow-y-auto">
            {isGenerating ? (
              <div className="flex flex-col items-center justify-center h-full text-center p-4">
                ...
                <p className="text-gray-600 dark:text-gray-400 font-medium">Génération de votre plan en cours...</p>
                <p className="text-sm text-gray-500 dark:text-gray-500 mt-2">Cela peut prendre quelques secondes</p>
              </div>
            ) : plan ? (
              <PlanView plan={plan} goal={currentGoal} />
            ) : (
              <div className="flex flex-col items-center justify-center h-full text-center p-4">
                <p className="text-gray-500 dark:text-gray-400">Aucun plan généré pour le moment.</p>
                <button onClick={() => setActiveTab('form')} className="mt-4 text-blue-600 dark:text-blue-400 font-medium">Créer un plan</button>
              </div>
            )}
          </div>
        )}
      ...
      <div className="fixed bottom-0 left-0 right-0 glass flex justify-around p-4 z-40 transition-all">
        ...
        >
          ...
          <span className={`${activeTab === 'form' ? 'block' : 'hidden'} text-sm`}>Objectif</span>
        </button>
        <button
        ...
        >
          ...
          <span className={`${activeTab === 'plan' ? 'block' : 'hidden'} text-sm`}>Plan</span>
        </button>
      </div>
    </Layout >
  )
